    ...
    if (product.length > 0) {
      const fetchPhotos = async () => {
        const urls = await Promise.all(
          product.map((item) =>
            axios.post(`${BASE_URL}/cover_image`, {
              fileName: item.cover,
            })
          )
        );
        setPhoto(urls.map((response) => response.data.action_product_image.url));
      };
      fetchPhotos();
    }
  }, [product]);
    ...
    <>
      ...
      <div className="flex justify-center w-11/12 lg:ml-20 ml-10">
        ...
        <div className="flex flex-nowrap w-full  ">
          <h4 className="text-2xl text-left tracking-normal mt-16 font-['Manrope'] font-bold">Philosophy</h4>
        </div>
      </div>
